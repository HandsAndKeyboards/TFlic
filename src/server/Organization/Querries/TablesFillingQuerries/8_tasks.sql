...
-----------------------------------------------------------------------
insert into tasks(column_id, position, name, description, creation_time, status) 
select c.id, 0 , 'Task', 'Типичное описание', DATE '2022-04-04', 'done'
from columns as c
...
insert into tasks(column_id, position, name, description, creation_time, status)
select c.id, 1 , 'Task1', 'Типичное описание', DATE '2022-04-04', 'done'
from columns as c
         inner join boards as b on b.id = c.board_id
         inner join projects as prj on b.project_id = prj.id
         inner join organizations as org on org.id = prj.organization_id
...
insert into tasks(column_id, position, name, description, creation_time, status)
select c.id, 2 , 'Task2', 'Типичное описание', DATE '2022-04-04', 'done'
from columns as c
         inner join boards as b on b.id = c.board_id
         inner join projects as prj on b.project_id = prj.id
         inner join organizations as org on org.id = prj.organization_id
...
insert into tasks(column_id, position, name, description, creation_time, status)
select c.id, 1 , 'Task3', 'Типичное описание', DATE '2022-04-04', 'done'
from columns as c
         inner join boards as b on b.id = c.board_id
         inner join projects as prj on b.project_id = prj.id
         inner join organizations as org on org.id = prj.organization_id
...
insert into tasks(column_id, position, name, description, creation_time, status)
select c.id, 0 , 'Task4', 'Типичное описание', DATE '2022-04-04', 'done'
from columns as c
         inner join boards as b on b.id = c.board_id
         inner join projects as prj on b.project_id = prj.id
         inner join organizations as org on org.id = prj.organization_id
...
insert into tasks(column_id, position, name, description, creation_time, status)
select c.id, 1 , 'Task5', 'Типичное описание', DATE '2022-04-04', 'done'
from columns as c
         inner join boards as b on b.id = c.board_id
         inner join projects as prj on b.project_id = prj.id
         inner join organizations as org on org.id = prj.organization_id
